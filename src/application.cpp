...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
	...
	SceneNode * node = new SceneNode("CT-Abdomen");
	...
	mesh->createCube();
	node->mesh = mesh;
	node->model.setScale(2, 2, 4);

	// Set material
	StandardMaterial* material = new StandardMaterial();
	material->shader = Shader::Get("data/shaders/basic.vs", "data/shaders/flat.fs");
	node->material = material;
	material->color = vec4(1.0, 1.0, 0.0, 1.0);

	//Volume
	Volume* volum = new Volume();
	volum->loadPVM("data/volumes/CT-Abdomen.pvm");

	//Texture

	Texture* texture = new Texture();
	texture->create3D(volum->width, volum->height, volum->depth, GL_RED, GL_UNSIGNED_BYTE, false, volum->data, GL_RED);

	material->texture = texture;
	material->shader = Shader::Get("data/shaders/basic.vs", "data/shaders/raymarching.fs"); //ara sense jittering
	material->noisetexture = Texture::Get("data/textures/blueNoise.png");
	//hide the cursor
...
{
	ImGui::Checkbox("Draw Grid", &render_debug);
